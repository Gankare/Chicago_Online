...
public class GameController : MonoBehaviour
{
    ...
    private bool drawingGambitCards = false;
    private string gambitWinnerId = "";
    ...
    async void PlayerTurnValueChanged(object sender, ValueChangedEventArgs args)
    {
    ...
    async void TurnTimerValueChanged(object sender, ValueChangedEventArgs args)
    {
        if (args != null && args.Snapshot != null && args.Snapshot.Value != null)
        {
    ...
    IEnumerator StartNextRound()
    {
        ...
        yield return UpdateLocalDataFromFirebase();
        yield return DisplayScore();
        ...
        {
            yield return ShuffleAndDealOwnCards(deck);
        ...
        {
            yield return UpdateCardButtons();
        ...
        yield return StartCoroutine(PlayerTurnTimer());
        yield break;
    }
    ...
    private IEnumerator PlayerTurnTimer()
    {
        ...
        if (currentGameState == (int)Gamestate.distributionOfCards)
        {
            while (Time.time - startTime <= turnDuration && !turnEndedEarly)
            {
                ...
                if (currentTurnTimer != previousTurnTimer && !turnEndedEarly) 
                {
            ...
            if (!turnEndedEarly)
            {
                yield return StartCoroutine(ThrowCards());
            }
                yield break;
        }
        else if (currentGameState == (int)Gamestate.gambit)
    ...
    IEnumerator EndPlayerTurn()
    {
        ...
        else if (currentGameState == (int)Gamestate.gambit)
        {
            ...
            if (getGambitRoundTask.Result.Exists)
            {
                ...
                if (currentGambitRound % playerCount == 0)
                {
                    ...
                    yield return FindGambitWinner();
                    if (userHandObjects.Count == 0 || userHandObjects == null)
                    {
                        var getPlayerScoreTask = DataSaver.instance.dbRef.Child("servers").Child(serverId).Child("players").Child(gambitWinnerId).Child("userGameData").Child("score").GetValueAsync();
                        ...
                        var setPlayerScore = DataSaver.instance.dbRef.Child("servers").Child(serverId).Child("players").Child(gambitWinnerId).Child("userGameData").Child("score").SetValueAsync(updatedScore);
                        yield return new WaitUntil(() => setPlayerScore.IsCompleted);

                        string username;
                        DatabaseReference userRef = DataSaver.instance.dbRef.Child("users").Child(gambitWinnerId);
                        ...
                        yield return ResetDeckAfterGambit();
                        var setGambitFalse = DataSaver.instance.dbRef.Child("servers").Child(serverId).Child("gameData").Child("gambit").SetValueAsync(false);
                    ...
                    if (!gameOver)
                    {
                        ...
                        yield return StartCoroutine(PassTurnToGambitWinner(gambitWinnerId));
                        yield break;
                    }
                    yield break;   
                }
        ...
        if (!gameOver)
        {
            ...
            yield return new WaitUntil(() => removeUserTurn.IsCompleted);
            yield return new WaitForSeconds(0.5f);
            yield return StartCoroutine(PassTurnToNextPlayer());
            yield break;
        }
        else
            yield break;
    }
    IEnumerator FindGambitWinner()
    {
        int higestCard = 0;
        ...
        if (cardsNeeded > 0)
        {
            for (int i = 0; i < cardsNeeded; i++)
            {
                if (shuffledDeck.Count == 0)
                {
                    ...
                    yield return DeckEmptyReShuffle();
                    yield break;
                }
    ...
    private IEnumerator UpdateLocalDataFromFirebase()
    {
        ...
        {
            yield return HandleGambit();
        }
        else
        {
            yield return HandleNonGambit();
        }
        Debug.Log("Loaded the datamaneten");
    }
    ...
    private IEnumerator HandleGambit()
    {
        ...
        if (int.Parse(getGambitRound.Result.Value.ToString()) == 0)
        {
            ...
            var setGambitSuitFalse = DataSaver.instance.dbRef.Child("servers").Child(serverId).Child("gameData").Child("gambitSuit").SetValueAsync("");
            yield return new WaitUntil(() => setGambitSuitFalse.IsCompleted);
            gambitSuit = "";
        }
        else
        {
            var getGambitSuit = DataSaver.instance.dbRef.Child("servers").Child(serverId).Child("gameData").Child("gambitSuit").GetValueAsync();
            yield return new WaitUntil(() => getGambitSuit.IsCompleted);
    ...
    IEnumerator ThrowCards()
    {
        if (!turnEndedEarly)
        {
            if (currentGameState == (int)Gamestate.distributionOfCards)
            {
                ...
                yield return ThrowCardsFromHand();
            }
            else if (currentGameState == (int)Gamestate.gambit && selectedCardObjects.Count > 0)
            {
                turnEndedEarly = true;
                endTurnButton.SetActive(false);
                if (selectedCardObjects.Count > 0)
                    yield return PutOutGambitCard();
                else
            ...
            yield return SendMessageThrowCards();
            yield return StartCoroutine(EndPlayerTurn());
            yield break;
        }
    }
    IEnumerator SendMessageThrowCards()
    {
        selectedCardObjects.Clear();
        if (currentGameState == (int)Gamestate.distributionOfCards)
    ...
    IEnumerator ThrowCardsFromHand()
    {
    ...
    IEnumerator PutOutGambitCard()
    {
        foreach (GameObject selectedCardObject in selectedCardObjects)
        {
            ...
            if (cardToRemove != null)
            {
                ...
                if (firstGambitCard)
                {
                    var setSuit = DataSaver.instance.dbRef.Child("servers").Child(serverId).Child("gameData").Child("gambitSuit").SetValueAsync(cardToRemove.cardSuit.ToString());
                    yield return new WaitUntil(() => setSuit.IsCompleted);
